...
const exp: string[] = ["{", "}", "{", "{", "}", "}"];
function balanced(exp: string[]): boolean {
    let expE: string[] = [];
...
function double(numbs: number): number {
...
function times100(numbrs: number): number {
...
export function div5(arr: number[]): number[] {
    let result: number[] = [];
    for (let x of arr) {
        if (x % 5 === 0) {
            result.push(x);
...
*/
export function calcDifferential(arr1: number[], arr2: number[]): number {
    let difference = 0;
